...
function GestionEncuestaProfesoresHU027() {
  ...
  const activeTemplateId = useMemo(() => {
    ...
    const activePeriod = normalizeValidPeriod(currentConfig?.semester || semester);
    const matchingTemplate = templates.find((template) => {
      const templatePeriod = normalizeValidPeriod(template.createdForSemester);
                    ...
                    return (
                      <article key={template.id}>
                        <div className="hu027-template-main">
                          <div className="hu027-template-title-row">
                            <strong>{template.name}</strong>
                            <span className={`hu027-template-badge ${isActiveTemplate ? 'active' : 'inactive'}`}>
